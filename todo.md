...
## 봇 애니메이션 개선
- [x] 봇 움직임 속도를 느리게 조정
- [x] 봇들이 순차적으로 움직이도록 변경 (한번에 이동하지 않음)
- [x] 봇 이동 후 위치 고정 (한번만 이동)
- [x] 봇들이 서로 겹치지 않도록 충돌 방지 로직 추가
- [x] 투표 주제 창에 남은 시간 타이머 표시
- [x] 결과 화면 1분간 노출 후 다음 라운드 진행

## UI/UX 강화
- [x] 모든 단계(질문 출제, 투표, 코멘트, 결과, 평가)에 타이머 표시
- [x] 봇 이동 경로를 베지어 곡선으로 변경하여 자연스러운 움직임 구현
- [x] 승리 진영 O/X 영역 전체에 네온 펄스 효과 추가
- [x] 승리 진영에 파티클 폭죽 애니메이션 추가

## 버그 수정
- [x] 봇들이 움직이지 않는 문제 해결 (베지어 곡선 애니메이션 로직 수정)

## 긴급 버그 수정
- [x] 게임 로딩이 안되는 문제 해결 (데이터베이스 외래키 오류 처리 추가)
- [x] 봇들이 움직이지 않는 문제 해결 (초기 위치 설정 로직 수정)
- [x] 라운드별 남은 시간 타이머가 활성화되지 않는 문제 해결 (모든 단계에 타이머 표시)

## 외부 AI 에이전트 통합
- [x] skill.md 파일 생성 (API 명세 및 사용법)
- [x] 봇 등록 REST API 엔드포인트 구현 (POST /api/trpc/agent.register)
- [x] WebSocket 연결 프로토콜 구현 (인증, 게임 상태 수신, 액션 전송)
- [x] 에이전트 인증 시스템 (API 키 기반)
- [x] 에이전트 액션 핸들러 (질문 제출, 투표, 코멘트, Heartbeat)
- [x] 테스트용 샘플 에이전트 코드 작성 (Python)
